...
public class Database {
	...
	static {
		rest.put((long) 0, new Restaurant("China Sushi", "Regenbogenstrasse 11", "06968", "cSushi", "s", true));
		rest.put((long) 1, new Restaurant("Japan Sushi", "Regenbogenstrasse 11", "06968", "cSushi", "s", true));
		rest.put((long) 2, new Restaurant("Nuclear Sushi", "Regenbogenstrasse 11", "06968", "cSushi", "s", true));
		...
		prodChinaShushi.put(product0.getId(), product0);
		prodChinaShushi.put(product1.getId(), product1);
		prodChinaShushi.put(product2.getId(), product2);
		prodChinaShushi.put(product3.getId(), product3);
	...
	public static Customer getCustomer(String email, String passwort) {
        ...
        for(Long key: keys){
           Customer local = cust.get(key);
           if(local.geteMail().equals(email) && local.getPasswort().equals(passwort))return local;
           else continue;
        //Iterating through the HashTable//
        }
        return null;
